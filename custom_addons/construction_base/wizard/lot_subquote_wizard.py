...
class LotSubquoteWizard(models.TransientModel):
    ...
    def _generate_preview_html(self, lines_info):
        ...
        html.append(f"<h5>📋 Contenu détecté pour le lot '{self.lot_id.name}' :</h5>")
        ...
        for line in lines_info['lines']:
            if line.display_type == 'line_section':
                html.append(f"<tr class='table-info'>")
                html.append(f"<td><strong>Section</strong></td>")
                html.append(f"<td><strong>{line.name}</strong></td>")
                html.append(f"<td>-</td><td>-</td>")
                html.append(f"</tr>")
            elif not line.display_type:
                html.append(f"<tr>")
                html.append(f"<td>Produit</td>")
                html.append(f"<td>{line.name}</td>")
                html.append(f"<td>{line.product_uom_qty}</td>")
                html.append(f"<td>{line.price_subtotal:,.2f} {line.currency_id.symbol}</td>")
                html.append(f"</tr>")
        
        html.append("</tbody></table>")
        html.append(f"<p><strong>Total estimé : {lines_info['total_amount']:,.2f} {self.currency_id.symbol}</strong></p>")
        html.append("</div>")
