# -*- coding: utf-8 -*-
"""
Wizard de prévisualisation et génération de contrats de sous-traitance.
Interface pour corriger les informations et voir la preview avant génération.
"""
...
class ContractPreviewWizard(models.TransientModel):
    """Wizard de prévisualisation et génération de contrats."""
    
    _name = 'construction.contract.preview.wizard'
    _description = 'Prévisualisation et génération de contrats de sous-traitance'
    ...
    preview_pdf = fields.Binary(
        string='Aperçu PDF',
        readonly=True,
        help="Aperçu du contrat en PDF"
    )
    
    preview_filename = fields.Char(
        string='Nom du fichier preview',
        readonly=True
    )
    ...
    def action_preview_contract(self):
        ...
        # Créer un contrat temporaire pour la preview
        ...
        temp_contract_vals = {
            ...
            'contract_number': contract_service._generate_contract_number(),
            'start_date': contract_data['start_date'],
            'end_date': contract_data['end_date'],
            'total_amount': contract_data['total_amount'],
            'payment_terms': contract_data['payment_terms'],
            'warranty_period': contract_data['warranty_period'],
            'insurance_required': contract_data['insurance_required'],
            'state': 'draft',
            'notes': contract_data['notes'],
        }
        
        # Créer le contrat temporaire
        temp_contract = self.env['construction.subcontractor.contract'].create(temp_contract_vals)
        
        # Générer le PDF de preview
        pdf_content = contract_service._generate_contract_pdf(temp_contract)
        
        # Mettre à jour le wizard avec la preview
        self.write({
            'preview_pdf': base64.b64encode(pdf_content),
            'preview_filename': f"Preview_Contrat_{self.subcontractor_id.name}_{self.chantier_id.name}.pdf"
        })
        
        # Supprimer le contrat temporaire
        temp_contract.unlink()
        
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('Aperçu généré'),
                'message': _('L\'aperçu du contrat a été généré avec succès.'),
                'type': 'success',
                'sticky': False,
            }
        }
