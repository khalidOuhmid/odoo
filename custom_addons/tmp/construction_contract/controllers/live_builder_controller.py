...
class ContractLiveBuilderController(http.Controller):
    ...
    @http.route('/contract/live-builder/<int:chantier_id>', type='http', auth='user', website=True)
    def live_builder(self, chantier_id=None, subcontractor_id=None, template_id=None, **kwargs):
        env = request.env
        user = env.user
        self._ensure_builder_access(user)

        if not chantier_id:
            return self._render_chantier_selector(env, error_message=None)

        chantier = env['construction.chantier'].browse(int(chantier_id))
        if not chantier.exists():
            return self._render_chantier_selector(
                env,
                error_message=_("The requested chantier could not be found or you do not have access to it.")
            )

        return self._render_builder_page(env, chantier, subcontractor_id, template_id)

    def _render_builder_page(self, env, chantier, subcontractor_id=None, template_id=None):
        """Render the live builder interface once a chantier is confirmed."""
        user = env.user
    ...
    def _render_chantier_selector(self, env, error_message=None):
        """Render a selection page so users can pick the chantier before entering the builder."""
        chantiers = self._get_available_chantiers(env)
        values = {
            'chantiers': chantiers,
            'error_message': error_message,
        }
        return request.render('construction_contract.contract_live_builder_selector', values)

    def _get_available_chantiers(self, env):
        """Return the chantiers the current user is allowed to edit."""
        domain = [('active', '=', True)]
        return env['construction.chantier'].search(domain, order='name asc')

    def _ensure_builder_access(self, user):
        """Ensure the current user can access the builder interface."""
