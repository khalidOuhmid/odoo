...
class ConstructionContract(models.Model):
    ...
    def _check_subcontractor_documents(self):
        """
        Validate that subcontractor has all required documents.
        Uses the validation_documents_service for comprehensive compliance checking.
        """
        validation_service = self.env['construction.validation.documents.service']
        ...
        for contract in self:
            ...
            if not (getattr(partner, 'siren', False) or partner.company_registry):
                raise ValidationError(_(
                    "Le sous-traitant '%s' doit avoir un numéro SIRET avant de créer un contrat."
                ) % partner.name)
            
            # Use validation service for comprehensive document checking
            compliance = validation_service.check_subcontractor_compliance(partner)
            
            if not compliance['is_compliant']:
                # Generate detailed error message
                error_message = validation_service.get_non_compliant_message(partner)
    ...
    def action_download_all_documents(self):
        """
        Download all contract documents as a ZIP archive.
        Includes: contract PDF + all deliverable attachments.
        File naming: [reference]_[type].[ext]
        
        Returns:
            dict: Action to download the ZIP file
        """
        self.ensure_one()
        ...
        if not self.pdf_document and not self.deliverable_ids:
            raise UserError(_("Aucun document disponible à télécharger."))
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            if self.pdf_document:
                contract_filename = f"{self.name.replace('/', '_')}_contrat.pdf"
                zip_file.writestr(contract_filename, base64.b64decode(self.pdf_document))
            
            # Add certificate if available
            if self.certificate_of_completion:
                cert_filename = f"{self.name.replace('/', '_')}_certificat.pdf"
                zip_file.writestr(cert_filename, base64.b64decode(self.certificate_of_completion))
            
            # Add deliverables
            for deliverable in self.deliverable_ids.filtered(lambda d: d.document):
                # Determine file extension from document name or type
                doc_name = deliverable.document_name or 'document'
                ext = doc_name.split('.')[-1] if '.' in doc_name else 'pdf'
                
                # Create filename: [reference]_[type].[ext]
                type_label = dict(deliverable._fields['deliverable_type'].selection).get(
                    deliverable.deliverable_type, deliverable.deliverable_type or 'annexe'
                )
                filename = f"{self.name.replace('/', '_')}_{type_label.replace(' ', '_')}.{ext}"
                ...
                zip_file.writestr(filename, base64.b64decode(deliverable.document))
        ...
        zip_filename = f"{self.name.replace('/', '_')}_documents.zip"
        ...
        _logger.info(
            f"ZIP archive created for contract {self.name}: "
            f"{len(zip_content)/1024:.1f} KB"
        )
    ...
    def action_send_email(self):
        ...
        Called from the enhanced post-creation summary view.
        Updates timeline after sending.
        """
        return self.action_send_portal_email()
    ...
    def action_send_sms(self):
        ...
        Updates timeline after sending.
        """
        return self.action_send_portal_sms()

    def action_send_both(self):
        """
        Send portal link via both email and SMS.
        Called from the enhanced post-creation summary view.
        Updates timeline after sending.
        """
        return self.action_send_portal_email_sms()
    ...
    @api.model
    def _cron_send_automatic_reminders(self):
        """
        Cron job to send automatic reminders for unsigned contracts
        Runs daily and sends reminders at J+7, J+14, and J+21 after initial sending
        
        Logic:
        - J+7 (7 days after sent_date): First reminder
        - J+14 (14 days after sent_date): Second reminder (urgent)
        - J+21 (21 days after sent_date): Final reminder (last warning)
        ...
        reminders = [
            (7, 1, 'J+7'),   # (days_after_sent, reminder_number, label)
            (14, 2, 'J+14'),
            (21, 3, 'J+21'),
        ]
        ...
        for days_after, reminder_number, label in reminders:
            # Calculate the target date range (e.g., 7 days ago +/- 12 hours for tolerance)
            target_date = now - timedelta(days=days_after)
            date_min = target_date - timedelta(hours=12)
            date_max = target_date + timedelta(hours=12)
            
            # Find contracts that:
            # 1. Are in 'sent' or 'in_progress' state
            # 2. Were sent approximately 'days_after' days ago
            # 3. Haven't received this specific reminder yet (or last reminder was sent before this threshold)
            
            # For first reminder (J+7): no previous reminder or last reminder was before J+7
            # For second reminder (J+14): last reminder was sent around J+7 (between J+6 and J+8)
            # For third reminder (J+21): last reminder was sent around J+14 (between J+13 and J+15)
            ...
            # Add condition based on last_reminder_date to avoid duplicate reminders
            if reminder_number == 1:
                # First reminder: no reminder sent yet, or last reminder was sent long ago (edge case)
                domain.append('|')
                domain.append(('last_reminder_date', '=', False))
                domain.append(('last_reminder_date', '<', now - timedelta(days=6)))
            else:
                # Subsequent reminders: last reminder should have been sent around the previous threshold
                previous_days = reminders[reminder_number - 2][0]  # Get previous threshold
                previous_min = now - timedelta(days=days_after - previous_days + 1)
                previous_max = now - timedelta(days=days_after - previous_days - 1)
                domain.extend([
                    ('last_reminder_date', '>=', previous_min),
                    ('last_reminder_date', '<=', previous_max),
                ])
            
            contracts = self.search(domain)
            
            _logger.info(f"Found {len(contracts)} contracts for {label} reminder")
            
            for contract in contracts:
                try:
                    # Send reminder with specific reminder number
                    result = notification_service.send_reminder(
                        contract,
                        reminder_number=reminder_number,
                        channel=contract.communication_channel
                    )
                    
                    if result.get('email_sent') or result.get('sms_sent'):
                        # Log activity
                        ActivityLog = self.env['construction.contract.activity']
                        ActivityLog.log_reminder_sent(contract, reminder_number)
                        
                        total_sent += 1
                        _logger.info(
                            f"✓ {label} reminder sent for contract {contract.name} "
                            f"(sent {days_after} days ago)"
                        )
                    else:
                        _logger.warning(
                            f"⚠️ {label} reminder failed for contract {contract.name}: "
                            f"{result.get('errors', [])}"
                        )
                        
                except Exception as e:
                    _logger.error(
                        f"✗ Failed to send {label} reminder for contract {contract.name}: {e}",
                        exc_info=True
                    )
        
        _logger.info(f"✓ Automatic reminders cron completed: {total_sent} reminders sent")
        return True

    @api.model
    def _cron_alert_expiring_documents(self):
        """
        Cron job to alert about expiring subcontractor documents.
        Runs weekly and notifies:
        - Responsable Administratif (all expiring documents)
        - Gestionnaires de Chantier (for their specific sites)
        
        Creates activities on impacted construction sites.
        ...
        # Get all partners with expiring documents (within 30 days)
        expiring_data = validation_service.get_expiring_documents(days_threshold=30)
        
        if not expiring_data:
            _logger.info("No expiring documents found")
            return True
        
        _logger.info(f"Found {len(expiring_data)} subcontractors with expiring documents")
        
        # Get Responsable Administratif group
        admin_group = self.env.ref(
            'construction_contract.group_contract_manager',
            raise_if_not_found=False
        )
        
        # Prepare summary for administrative team
        summary_lines = [
            _("Documents sous-traitants expirant dans les 30 prochains jours :"),
            ""
        ]
        
        for partner_data in expiring_data:
            partner_id = partner_data['partner_id']
            partner_name = partner_data['partner_name']
            expiring_docs = partner_data['expiring_documents']
            
            summary_lines.append(f"• {partner_name}:")
            for doc in expiring_docs:
                days = doc['days_until_expiry']
                summary_lines.append(
                    f"  - {doc['document_name']}: expire dans {days} jour(s) "
                    f"({doc['expiry_date']})"
                )
            summary_lines.append("")
            
            # Find all active construction sites involving this subcontractor
            chantiers = self.env['construction.chantier'].search([
                ('subcontractors', 'in', partner_id),
                ('state', 'not in', ['done', 'cancelled'])
            ])
            
            # Create activity on each impacted construction site
            for chantier in chantiers:
                # Get gestionnaires for this chantier
                gestionnaires = chantier.user_ids if hasattr(chantier, 'user_ids') else []
                
                activity_note = _(
                    "Documents du sous-traitant %(partner)s expirant bientôt :\n"
                ) % {'partner': partner_name}
                
                for doc in expiring_docs:
                    activity_note += f"• {doc['document_name']}: {doc['expiry_date']}\n"
                
                # Create activity for gestionnaires
                if gestionnaires:
                    for user in gestionnaires:
                        self.env['mail.activity'].create({
                            'res_model_id': self.env['ir.model']._get_id('construction.chantier'),
                            'res_id': chantier.id,
                            'activity_type_id': self.env.ref('mail.mail_activity_data_warning').id,
                            'summary': _("Documents sous-traitant expirant"),
                            'note': activity_note,
                            'user_id': user.id,
                        ...
                        _logger.info(
                            f"Activity created for user {user.name} on chantier {chantier.name}"
                        )
        
        # Send summary email to administrative team
        if admin_group and admin_group.users:
            summary_body = "<br/>".join(summary_lines)
            
            for admin_user in admin_group.users:
                if admin_user.email:
                    # Create mail message
                    mail_values = {
                        'subject': _("Alerte : Documents sous-traitants expirant"),
                        'body_html': f"<p>{summary_body}</p>",
                    ...
                    _logger.info(f"Alert email sent to {admin_user.name} ({admin_user.email})")
        
        _logger.info(
            f"✓ Expiring documents alert cron completed: "
            f"{len(expiring_data)} subcontractors processed"
        )
        return True
