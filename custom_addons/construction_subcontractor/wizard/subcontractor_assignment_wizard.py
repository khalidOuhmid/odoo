...
class SubcontractorAssignmentWizard(models.TransientModel):
    ...
    partner_id = fields.Many2one('res.partner', string='Sous-traitant', required=True, domain="[('supplier_rank', '>', 0)]")
    ...
    def _compute_validation(self):
        for wiz in self:
            ...
            if wiz.partner_id.doc_insurance_dec_status != 'valid':
                reasons.append(_("❌ Assurance Décennale invalide ou manquante"))
            
            # Check other required docs
            if wiz.partner_id.doc_kbis_status != 'valid':
                reasons.append(_("⚠️ KBIS invalide"))
            
            if wiz.partner_id.doc_urssaf_status != 'valid':
