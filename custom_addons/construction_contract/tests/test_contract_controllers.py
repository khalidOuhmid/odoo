...
class TestContractControllers(common.HttpCase):
    ...
    def test_11_live_builder_selector_available(self):
        """Ensure the builder selector page renders without chantier_id"""
        self.authenticate('admin', 'admin')
        response = self.url_open('/contract/live-builder')
        self.assertEqual(response.status_code, 200, "Selector should render for allowed users")

    def test_12_live_builder_invalid_chantier_fallback(self):
        """Invalid chantier should fall back to selector with message"""
        self.authenticate('admin', 'admin')
        response = self.url_open('/contract/live-builder?chantier_id=999999')
        self.assertEqual(response.status_code, 200, "Invalid chantier should not crash the page")
