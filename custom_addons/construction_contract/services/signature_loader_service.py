...
class SignatureLoaderService(models.AbstractModel):
    ...
    @api.model
    def validate_signature_image(self, image_data):
        """
        ...
        try:
            ...
            if not (is_png or is_jpeg):
                _logger.warning("Signature image is not PNG or JPEG format")
                return False
            
            _logger.debug(
                f"Signature validation passed: "
                f"{'PNG' if is_png else 'JPEG'}, {size_mb:.2f} MB"
            )
